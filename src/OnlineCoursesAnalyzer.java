...
public class OnlineCoursesAnalyzer {
    ...
    public Map<String, Integer> getPtcpCountByInstAndSubject() {
        ...
        result = result.entrySet().stream().sorted(Collections.reverseOrder(Map.Entry.comparingByValue()))
        ...
        if (by.equals("hours")) {
            ...
            for (Course course : courses) {
                if (titleTime.containsKey(course.title)) {
                    titleTime.put(course.title, Math.max(titleTime.get(course.title),course.totalHours));
                } else {
                    titleTime.put(course.title, course.totalHours);
                }
            }
            titleTime = titleTime.entrySet().stream().sorted(Collections.reverseOrder(Map.Entry.comparingByValue()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (s, s2) -> s,
                    LinkedHashMap::new));
            ...
            titlePart = titlePart.entrySet().stream().sorted(Collections.reverseOrder(Map.Entry.comparingByValue()))
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (s, s2) -> s,
                    LinkedHashMap::new));
    ...
    public List<String> recommendCourses(int age, int gender, int isBachelorOrHigher) {
        ...
        for (Entry<String, List<Double>> entry : idInfoTable.entrySet()) {
            ...
            double similarityvalue = Math.pow(age - averageAge, 2) + Math.pow(gender * 100 - averageMale, 2)
